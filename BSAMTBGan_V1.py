...
class BSMTBGan(object):
    def __init__(self,global_Dir,user_ref,user_ref_fasta,File_table,mappers):
      ...
      #Settings parameters and paths
        ...
        self.ref_gen_Dir="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/Reference/"#H37rv reference
        self.genome=user_ref
        self.fasta=user_ref_fasta
        self.ref_genome="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/Reference/FASTA/"#fasta file of reference genome
        self.ref="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/Reference/FASTA/"+self.fasta#fasta file of reference genome
        ...
        self.dbSNP="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/Reference/dbSNP/dbSNP.vcf"
        self.illumina_adapters="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/illumina_adapters.fna.fasta"
        self.fastQFileData=File_table
        self.gtfToGenePred="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/gtfToGenePred"#bin file
        self.H37RV_gtf="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/Mycobacterium_tuberculosis_h37rv.ASM19595v2.36.gtf"
        self.TBdb="/Users/sondeskalboussi/Desktop/BSAMGan_new_pipeline/dbMTB"
        self.spotyping=self.tools+"/SpoTyping-2.1/SpoTyping-v2.1-commandLine/"
        self.Sample=[]
        self.multi_libraryPE={}
        self.multi_librarySE={}
    ...
    # Create a dictionary where the keys are samples ID and value is the reads ID when they are coming from more than sequencing library so later you can merge all the output for the same sample 
        for fl in os.listdir(self.results+"Trimming/"):
                        ...
                        if fl.endswith("R1.PE_paired_trimed.fq"):
                                    ...
                                    sm=IDPE.split("_")[0]
                                    lib=IDPE.split("_")[2]
                                    if sm not in self.multi_libraryPE.keys():
                                        self.multi_libraryPE[sm]=[lib]
                                    else:
                                        self.multi_libraryPE[sm].append(lib)
                        if fl.endswith("_SE_trimed.fq"):
                                    IDSE=fl.split("_SE_trimed.fq")[0]
                                    sm=IDSE.split("_")[0]
                                    lib=IDSE.split("_")[2]
                                    if sm not in self.multi_librarySE.keys():
                                        self.multi_librarySE[sm]=[lib]
                                    else:
                                        self.multi_librarySE[sm].append(lib)
        ...
        for map in self.mappers:           
            for fl in os.listdir(self.results+map+"/Alignment/"):
                  ...
                  if fl.endswith("_dedup.bam"):
                        ...
                        os.system("""java -jar {} -T RealignerTargetCreator -nt 8 -R {} -I {} -o {}""".format(self.GATK,self.ref,input,output1))#change thread
        ...
        for map in self.mappers:
             ...
             for fl in os.listdir(self.results+map+"/Alignment/"):
                  ...
                  if fl.endswith(map+"_realg_sorted.bam"):
                        ...
                        os.system("""java -jar {} -T BaseRecalibrator -nct 8 -R {} -I {} -knownSites {} -o {}""".format(self.GATK,self.ref,input,self.dbSNP,output1))#add path to GATK cluster
                        os.system("""java -jar {} -T PrintReads -nct 8 -R {} -I {} -BQSR {}  -o {}""".format(self.GATK,self.ref,input,output1,output2))
                        os.system("""samtools sort -o {} {}""".format(output3,output2))
                        os.system("""java -jar {} BuildBamIndex INPUT={}""".format(self.picard,output3))
    ...
    def merge_bam(self):
        for map in self.mappers:                
            #samples that do need merging are moved to a new final folder if sample id has one lib value
                for key,value in self.multi_libraryPE.iteritems():
                    if len(value)==1:#no merging just move bam file to final_Bam folder
                        for i in value:
                            os.system("cp "+self.results+map+"/Alignment/"+key+"_"+i+"_PE_"+map+"_recal_sorted "+self.results+map+"/Alignment/Final_Bam/"+key+"_"+i+"_PE_"+map+"_recal_sorted.bam")
                      ...
                      ====================================
                         ...
                         to_be_mergedPE=""
                         for i in value:
                            multi_libraryPE.append(self.results+map+"/Alignment/"+key+"_"+i+"_PE_"+map+"_recal_sorted")
                         ...
                         os.system(cmdPE)
                for key,value in self.multi_librarySE.iteritems():
                    if len(value)==1:
                        for i in value:
                      ...
                      =============================================
                        ...
                        to_be_mergedSE=""
                        for i in value:
                            multi_librarySE.append(self.results+map+"/Alignment/"+key+"_"+i+"_PE_"+map+"_recal_sorted.bam")
                                        #when i find the id in the file name write the file name multi
                ...
                for fl in os.listdir(self.results+map+"/Alignment/Final_Bam/"):
                            ...
                            os.system("""samtools sort -o {} {}""".format(fl.replace(".bam","_sorted.bam",fl)))
            ...
            for map in self.mappers:
                ...
                for fl in os.listdir(self.results+map+"/Alignment/Final_Bam"):
                        ...
                        if fl.endswith("_sorted.bam"):
                            input=os.path.join(self.results+map+"/Alignment/Final_Bam",fl)
    ...
    def joint_variant_calling(self):
            ...
            for map in self.mappers:
                ...
                for i in self.valide_bam:
                 for fl in os.listdir(self.results+map+"/Alignment/Final_Bam"):
                    if i in fl and fl.endswith(".bam"):
                        os.chdir(self.results+map+"/Alignment/Final_Bam")
                        input=os.path.join(self.results+map+"/Alignment/Final_Bam",fl)
                        output=self.results+map+"/Joint_variants/"+i+"_GATK_snps_indels.vcf"
                        ...
                        #-ploidy 1 --emitRefConfidence BP_RESOLUTION
    ...
    def joint_variant_calling_hard_filtering(self):
         for map in self.mappers:
             for fl in os.listdir(self.results+map+"/Joint_Variants"):
                 ...
                 for i in self.valide_bam:
                     output1=self.results+map+"/Joint_variants/"+i+"_raw_snps.vcf"
                     output11=self.results+map+"/Joint_variants/"+i+"_filtered_snps.vcf"
                     output2=self.results+map+"/Joint_variants/"+i+"_raw_indels.vcf"
                     output22=self.results+map+"/Joint_variants/"+i+"_filtered_indels.vcf"
                     #variant selection
...
#return "VCF statistics is done"
    ...
    def Genotype_structural_variation_calling(self):
        ...
        for map in self.mappers:
            ...
            for fl in os.listdir(self.results+map+"/Alignment/Final_Bam"):
                ...
                #print """delly call -t DEL -g {} -o {}.bcf {}""".format(self.ref,output,input)
    ...
    def Gene_annotation_annovar(self):
        ...
        #run annovar gene annotation():
                            os.system("""perl {}table_annovar.pl {} {} --vcfinput --outfile {} -buildver H37Rv --protocol refGene -operation g """.format(self.annovar,input,self.TBdb,i+"_"+map))
                            os.system("""mv {} {}""".format(self.results+map+"/Joint_Variants/"+i+"_"+map+".H37Rv_multianno.vcf",self.results+map+"/Annotation/"+i+"_"+map+".H37Rv_multianno.vcf"))

        return " Annovar Gene annotation is done!"
        
def main():
        pipeline=BSMTBGan("/Users/sondeskalboussi/Desktop/BSAMGan_test1","Mycobacterium_tuberculosis_h37rv","Mycobacterium_tuberculosis_h37rv.fa","/Users/sondeskalboussi/Desktop/Fastq.csv", mappers=["BWA","Novoalign"])
        try:

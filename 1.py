...
class BSMTBan(object):
    def __init__(self,Novoalign,BWA_mem,BWA_mem_map_output,novo_align_map_output,ref_genome,ref_gen_Dir,Trimmomatic,bwa,novoalign,GATK,fastq_Dir,dbSNP,illumina_adapters,samtools,picard,bedtools,bcftools,Delly,annovar,gtfToGenePred,MTdb,H37RV_gtf)
      ...
      self.self.TBdb=MTdb
      self.H37RV_gtf=H37RV_gtf# H37RV.gtf file
           ...
           """
            ...
            if not os.path.isdir(self.TBdb):
                ...
                convert2annovar.pl -format vcf4 example/ex2.vcf > ex2.avinput
                    python -c "import sys;lines=sys.stdin.read();print lines.replace('gi|444893469|emb|AL123456.3|','Chromosome')" < /Users/sondeskalboussi/Desktop/test.avinput > new            

            #os.system("wget ftp://ftp.ensemblgenomes.org/pub/bacteria/release-36/fasta/bacteria_0_collection/mycobacterium_tuberculosis_h37rv/dna/   -P "+self.TBdb) 
            #os.system("gunzip "+self.annotation_db+"/Mycobacterium_tuberculosis_h37rv.ASM19595v2.dna.chromosome.Chromosome.fa.gz")
            #convert gene definition file into gene prediction file
            for fl in os.listdir(self.annotation_db):
                    if not os.path.exists(self.annottion_db+"/H37Rv_refGene.txt")  and not os.path.exists(self.TBdb+"/H37Rv_refGeneMrna.fa"):                
                        os.system("""./{} -genePredExt {} {}""".format(self.gtfToGenePred,self.H37RV_gtv,self.TBdb+"/H37Rv_refGene.txt")) #run a bin file (first check if you have lib2 installed  
                ...
                if self.BWA_mem:
                    ...
                    for fl in os.listdir(self.BWA_mem_map_output+"/Joint_Variants/"):
                            ...
                            for id in sample:
                                ...
                                os.system("""perl {}/table_annovar.pl --vcfinput {} {} --outfile {} --buildver H37Rv --nastring . --protocol refGene,dbsnp --operation g,r,f  --vcfinput""".format(self.annovar,input,self.TBdb,output)
                if self.Novoalign:

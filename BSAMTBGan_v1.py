...
class BSMTBan(object):
    def __init__(global_Dir,ref_gen_Dir,user_ref,fastq_Dir,File_table,mappers)
	#mappers is a list of mapper we want to use
      #Tool settings and paths
          self.global_Dir=global_Dir# path to CWGSMTB directory in the cluster
          ...
          self.fastQFileData=self.global_Dir+"/"+File_table 
...
#                        os.system("java -jar {}picard.jar CreateSequenceDictionary R={}.fasta O={}.dict""".format(self.picard,fl,ID))
 #           return " "  
   ...
   def realignment(self):          
    ...
    def merge_bam(self):
        #check if there is samples from more than one library if yes merge them and move them to Final alignment otherwise use bam file in Alignment directory with the extension final_mapper.bam                  
        #samples that do not need merging
        for map in mappers:     
            if not all(len(multi_libraryPE[k]) == f for k in multi_libraryPE):   
             #check if all the samples have one library so no need to merging, else merge files from different library
                continue 
                print ("""            
                      ==============================================
            ...
            if not all(len(multi_librarySE[k]) == f for k in multi_librarySE):   
             #check if all the samples have one library so no need to merging, else merge files from different library
                continue 
                print ("""            
                      ==============================================
    ...
    def genome_coverag_mappability_stat(self):
            ...
            for map in mappers:
                ...
                #check if i have merged files or not and call the corresponding folder    
                for fl in os.listdir(self.results+"/"+map+"/Alignment/Final_Bam"):
                    ...
                    if fl.endswith("_merged_bwa.bam") or fl.endswith("_final_bwa.bam")      
                                  input=self.results+"/"+map+"/Alignment/Final_Bam/"+fl
                                  for ID in Sample:
    ...
    def joint_variant_calling(self):
        ...
        for map in mappers:
                    ...
                    for fl in os.listdir(sself.results+"/"+map+"/Alignment/Final_Bam"):
                            os.chdir(self.results+"/"+map+"/Alignment/Final_Bam")
                            for id in sample:
                                input=self.results+"/"+map+"/Alignment/Final_Bam/"+fl
    ...
    def structural_variation_calling(self):
        ...
        for map in mappers:
                    ...
                    for fl in os.listdir(self.results+"/"+map+"/Alignment/Final_Bam"):
                            os.chdir(self.results+"/"+map+"/Alignment/Final_Bam")
                            for id in sample:
                                input=self.results+"/"+map+"/Alignment/Final_Bam/"+fl#sorted indexed
    ...
    def Gene_annotation_annovar(self):
        ...
        for map in mappers:        
            ...
            #convert gene definition file into gene prediction file
              #First Filter out all SNP in repetitive region in the genome
...
if __name__ == '__main__':
    ...
    pipeline=mapping_to_ref_genome_functional_annotation(Novoalign,"+map+","+map+"_map_output,novo_align_map_output,ref_genome,ref_gen_Dir,Trimmomatic,bwa,novoalign,GATK,fastq_Dir,dbSNP,illumina_adapters,samtools,picard,bedtools,bcftools,Delly,annovar,gtfToGenePred,MTdb,H37RV_gtf)                                      
